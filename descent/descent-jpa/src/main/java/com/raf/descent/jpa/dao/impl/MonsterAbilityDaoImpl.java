...
import com.raf.descent.jpa.dao.MonsterAbilityDao;
import com.raf.descent.jpa.domain.card.MonsterAbility;
import com.raf.descent.jpa.domain.card.MonsterAbilityPk;

/**
 * Implementation DAO for {@link MonsterAbility}.
 * 
 * @author RAF
 */
@Repository
public final class MonsterAbilityDaoImpl extends AbstractDao<MonsterAbility, MonsterAbilityPk> implements
    MonsterAbilityDao {

  /**
   * Constructor.
   */
  public MonsterAbilityDaoImpl() {
    super(MonsterAbility.class);
  }
   ...
   * @param example
   *          the example
   * @see AbstractDao#getPredicates(Root, MonsterAbility)
   */
  @Override
  protected Predicate[] getPredicates(final Root<MonsterAbility> root, final MonsterAbility example) {
   ...
   *          the predicate list
   */
  private void addMonsterStatFilter(final Root<MonsterAbility> root, final MonsterAbility example,
   ...
   */
  @Override
  protected List<Order> getOrder(final CriteriaBuilder builder, final Root<MonsterAbility> root) {
    final List<Order> orders = new ArrayList<Order>();
    orders.add(builder.asc(root.get(IDENT).get("monsterStatPk")));
    orders.add(builder.asc(root.get(IDENT).get("ability")));
    return orders;
  }
